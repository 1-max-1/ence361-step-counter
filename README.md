...
- Task modules – these modules represent the tasks which the scheduler executes. They expose only init() and execute/update() functions which runs the logic for the task. 
- State logic modules – these modules run the logic for each of the display states like current steps or distance travelled. They are at a lower level than the task logic modules.
...
The step data module stores user data such as the number of steps taken, and the currently selected display units. The state machine module tracks the program display state and provides methods for switching between states.
...
- usartPrintingTask – this task was only used during debugging. It will print certain values to the serial port.
- goalProgressIndicatorTask – this task tracks the user’s goal progress and will indicate progress using LED’s, the buzzer and haptic feedback.
- stepTrackTask – this task contains the step detection algorithm and will update the number of steps the user has taken.
- displayTask – this task decides which display function to run, as this is dependent on the program state. The display functions are defined in the uiDisplay module (see section below). This task does not interact with the screen; all graphics are handled in the uiDisplay module. This was done to decouple the graphics code from the current state decision making code.
...
### Hardware modules
These modules expose functions to control the hardware components on the nucleo and RCAP boards. They hide the low-level implementation so that changing the way hardware is accessed does not break any of the tasks or other modules. The hardware modules mostly use the stm32 HAL layer functions. Hardware modules exist for the following components:
- ADC
- Joystick
- Push buttons
- Potentiometer
...
The LED and buzzer modules use an additional PWM module to vary the duty cycle of the PWM generation. This results in changing the buzzer volume or the LED brightness.
...
The uiDisplay module handles graphics and UI layout. Each display state has a similar layout but different text. The module handles building the strings from the step data and using the relevant hardware module to write to the screen. The uiDisplay module is separate from the displayTask module, which decides which function to call. This was done to decouple the decision making (dependent on state machine implementation) from the graphics code.
...
The buffer length for the average calculation also had to be tuned. A buffer that was too short would cause the average to follow the data too closely and would result in large fluctuations. A buffer that was too long would cause too much lag in the average and it would not follow the data closely enough, resulting in an invalid step count.

 
# Results & Analysis
To analyse the performance and accuracy of the step tracking algorithm, data was sent via serial comms to a python logging program on a lab computer. This tracked the current moving average, filtered IMU magnitude, current variance, points where the magnitude crossed the average, and detected steps. This was plotted in two graphs. Figure 2 shows these graphs for a single test of eight steps (open image in new tab or visit link for full size image).

![Image](https://github.com/user-attachments/assets/7f6dd4b0-bd42-4352-9869-2700fd32870e)

analysing these graphs shows that for the test, the magnitude of the IMU data crosses the moving average many times. However, since the algorithm also checks against variance magnitude, time since last detected step, and peak height, most of these crossings were filtered out, leaving only valid crossings to be detected as steps. As can be seen in Figure 2, the algorithm was able to successfully detect all eight steps without additional misdetections. Multiple tests were conducted to determine the robustness and accuracy of the algorithm, results were positive, with a few outliers in edge case scenarios such as running and exceptionally light stepping. For all other walking tests such as single step tests and long-distance tests the device was able to accurately detect the number of steps taken up to a minimum of 200 steps, with a precision of >95%. This level of accuracy was deemed to be adequate for this project given the specifications given. 

While the step detection was found to be acceptable accurate, a few improvements could be made to improve accuracy in edge case scenarios as mentioned before. This would mostly involve tuning the algorithm for running and light stepping as these cases would result in unusual peak formations and caused the algorithm to struggle to detect all steps taken. For running, steps often occurred too quickly and would be filtered out by the step detection time constraint. Decreasing this would have allowed for a more accurate step count while running. However, since running was not within the scope of this project, this was not done to preserve the accuracy of the walking step detection. For light stepping cases the peaks formed would not be high enough to meet the peak height requirements. However, this was also deemed okay as for this to be a problem the steps taken had to be unreasonably light and were also considered to be out of the scope of this project as it involved walking in a very unnatural manner.

Aside from the step detection algorithm, the rest of the software performed robustly meeting all requirements and specifications outlined for this project. There was one small bug found which occurred when using the joystick to set the current steps while in test mode. Where the joystick would not start incrementing steps until a certain percentage of displacement was reached. This would normally be fine however, the specific displacement required appeared to be related to the size of the current goal. For example, when the goal was set to 500 steps, the joystick would have to reach ~60% displacement before the value of the steps would begin to change. Whereas with a high goal such as 15000 steps, steps would begin to change with a joystick displacement of as little as ~5%. This is a minor bug and did not affect any functions of the device, merely the user’s experience of setting the steps taken. Other than this, no other bugs were found and the software performed as expected in all testing completed, highlighting it’s robustness.
